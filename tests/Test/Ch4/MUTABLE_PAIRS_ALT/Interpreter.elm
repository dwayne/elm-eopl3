module Test.Ch4.MUTABLE_PAIRS_ALT.Interpreter exposing (suite)

import Ch4.MUTABLE_PAIRS_ALT.Interpreter as I exposing (Value(..))
...
suite : Test
suite =
    describe "Ch4.MUTABLE_PAIRS_ALT.Interpreter" <|
        List.map testValue
            [ ( "5"
              , VNumber 5
              )
            ...
            , ( "let p = pair(1, 2) in left(p)"
              , VNumber 1
              )
            , ( "let p = pair(1, 2) in right(p)"
              , VNumber 2
              )
            , ( """
                let p = pair(1, 2)
